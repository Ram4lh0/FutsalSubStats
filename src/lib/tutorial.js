...
export const guidedTutorialSteps = [
  ...
  {
    // Ações e cartões dos atletas: uma só paragem, com um card curto de "o
    // que fazer" — o resto do intervalo ali dentro só repetia o que o card
    // já dizia e tapava os botões da própria ação.
    id: 'live',
    titleKey: 'tutorial.guiado.acoesTitulo',
    actionKey: 'tutorial.guiado.acoesAcao',
    target: '[data-tour="live-clock"], [data-tour="live-court"]',
    autoHideMs: 20000,
  },
  {
    id: 'halftime',
  ...
  {
    // Antes ia daqui para a home e depois para o cartão do escalão (que
    // reabria o plantel). Hoje as estatísticas e o dashboard vivem na aba
    // Análise da navegação principal — é para lá que se aponta.
    id: 'summaryHome',
    titleKey: 'tutorial.guiado.analiseTitulo',
    textKey: 'tutorial.guiado.analiseTexto',
    target: '[data-tour="nav-analysis"]',
    // A navegação principal está sempre no ecrã (em quase todas as páginas),
    // ao contrário dos alvos dos outros passos, que só aparecem na página
    // certa. Por isso este passo não pode ser avaliado pelo mecanismo
    // genérico de "o alvo já está visível, avança/mantém": estaria sempre
    // visível, mesmo no primeiro passo do tutorial. A entrada e a saída
    // deste passo são explícitas (ver MainNavigation.jsx).
    manual: true,
  },
  {
    id: 'stats',
