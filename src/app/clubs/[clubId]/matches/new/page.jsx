...
import { clubs, players, matches, squad, events } from '@/lib/data/repository.js';
...
function Assistente() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setRoster((await players.listByClub(clubId)).filter((p) => p.isActive));
    })();
  }, [clubId]);
  ...
  if (!roster.length) {
    return (
      <>
        <PageHead title="Novo jogo" backTo={`/clubs/${clubId}`} />
        <Empty
          action={
            <button
              className="btn btn--primary"
              onClick={() => router.push(`/clubs/${clubId}/players/new`)}
            >
              Criar jogador
            </button>
          }
  ...
  async function guardar(abrir) {
    ...
    setAGuardar(true);
    try {
      const jogo = await matches.create(clubId, {
      ...
      router.push(abrir ? `/matches/${jogo.id}/setup` : `/clubs/${clubId}/matches`);
    } catch (err) {
  ...
  const nav = (voltarLabel, seguinteLabel, aoSeguinte) => (
    <div className="wizard__nav">
      <button
        ...
        type="button"
        onClick={() => (etapa === 0 ? router.push(`/clubs/${clubId}/matches`) : setEtapa(etapa - 1))}
      >
        {voltarLabel}
      </button>
  ...
  return (
    <>
      <PageHead title="Novo jogo" subtitle={club?.name} backTo={`/clubs/${clubId}/matches`} />
        ...
        {etapa === 0 ? (
          <div className="card form">
            ...
            <Field label="Competição">
              <input className="input" placeholder="Campeonato, taça, particular…" {...campo('competition')} />
            </Field>
            <div className="form__row">
              <Field label="Local">
                <input className="input" placeholder="Pavilhão" {...campo('venue')} />
              </Field>
              <Field
                label="Tempo de jogo"
                hint="Definido no clube, igual para todos os jogos desta equipa."
              >
                <p className="readonly">{MATCH_TIMING_LABEL[timingOf(club)]}</p>
              </Field>
            </div>
            <Field label="Notas">
    ...
    <div className="card">
      <dl className="review">
        ...
        <div>
          <dt>Local</dt>
          <dd>
            {HOME_AWAY_LABEL[info.homeOrAway]}
            {info.venue ? ` · ${info.venue}` : ''}
          </dd>
        </div>
        <div>
          <dt>Competição</dt>
          <dd>{info.competition || '—'}</dd>
        </div>
        <div>
          <dt>Tempo de jogo</dt>
          <dd>{MATCH_TIMING_LABEL[timingOf(club)]}</dd>
        </div>
        <div>
          <dt>Convocados</dt>
